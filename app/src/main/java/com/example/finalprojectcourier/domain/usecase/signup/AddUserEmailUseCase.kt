...
class AddUserEmailUseCase @Inject constructor(private val additionalUserDataRepository: FirebaseAdditionalUserDataRepository) {
    suspend operator fun invoke(data: GetUserAdditionalData) : Flow<Resource<Boolean>> {
        return additionalUserDataRepository.addUserEmail(data)
    }
}